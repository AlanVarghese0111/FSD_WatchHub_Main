...
import { Typography, Button, Select, MenuItem, Container, Grid, Paper, Box } from "@mui/material";
    ...
    <div>
      ...
      <div style={{ marginLeft: 240, padding: '20px' }}> {/* Adjust the margin to accommodate the sidebar */}
        ...
        <header style={{ backgroundColor: '#333', color: '#fff', padding: '20px', textAlign: 'center', borderRadius: '8px', marginBottom: '20px' }}>
          <h1 style={{ fontSize: '32px', marginBottom: '10px' }}>View Orders</h1>
            ...
            {orders.map((order) => (
              <Grid item xs={12} key={order._id}>
                <Paper elevation={3} style={{ padding: '20px' }}>
                  ...
                  <Typography variant="body1">First Name: {order.firstName}</Typography>
                  <Typography variant="body1">Last Name: {order.lastName}</Typography>
                  <Typography variant="body1">Address: {order.address}</Typography>
                  <Typography variant="body1">Pincode: {order.pincode}</Typography>
                  <Typography variant="body1">Landmark: {order.landmark}</Typography>
                  ...
                  {editingOrderId === order._id ? (
                    ...
                    <Button
                      ...
                      size="small"
                      style={{ backgroundColor: '#f57c00', color: '#fff', marginTop: '10px' }}
                    >
                      Edit
                    </Button>
                  )}
